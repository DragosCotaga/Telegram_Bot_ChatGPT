...
# OpenAI secret Key
API_KEY = 'INSERT API KEY FROM CHATGPT'
# Models: text-davinci-003,text-curie-001,text-babbage-001,text-ada-001
MODEL = 'text-davinci-002'
# Telegram secret access bot token
BOT_TOKEN = '<INSERT BOT TOKEN FROM TELEGRAM>'
...
def openAI(prompt):
    ...
    response = requests.post(
        'https://api.openai.com/v1/engine/completions/' + MODEL,
        headers={'Content-Type': 'application/json', 'Authorization': 'Bearer ' + API_KEY},
        json={'prompt': prompt, 'max_tokens': 1024, 'temperature': 0.7, 'n': 1, 'stop': '\n'}
    )
...
def Chatbot():
    ...
    # Running the function again after a 5-second delay
    threading.Timer(5, Chatbot).start()
